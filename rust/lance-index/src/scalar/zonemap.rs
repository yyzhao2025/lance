...
impl ZoneMapIndex {
    ...
    fn zone_has_extrema(zone: &ZoneMapStatistics) -> bool {
        !(zone.min.is_null()
            || zone.max.is_null()
            || Self::scalar_is_nan(&zone.min) && Self::scalar_is_nan(&zone.max))
    }
                    ...
                    if value.is_null() {
                        ...
                        match value {
                            ScalarValue::Float16(Some(f)) => {
                                if f.is_nan() {
                                    zone.nan_count > 0
                                } else if !Self::zone_has_extrema(zone) {
                                    false
                                } else {
